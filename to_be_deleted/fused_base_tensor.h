...
// Base class: FusedBaseTensor
template <typename T, my_size_t... Dims>
class FusedBaseTensor : public BaseExpr<FusedBaseTensor<T, Dims...>, T>
{
public:
    // Default constructor
    FusedBaseTensor()
    ...
    // Copy constructor
    FusedBaseTensor(const FusedBaseTensor &other)
    {
    ...
    // Move constructor
    FusedBaseTensor(FusedBaseTensor &&other) noexcept
    {
#ifdef DEBUG_FUSED_TENSOR
    ...
    FusedBaseTensor &operator=(const BaseExpr<Expr, T> &expr)
    {
#ifdef DEBUG_FUSED_TENSOR
        std::cout << "FusedBaseTensor assignment operator called" << std::endl;
#endif
    ...
    FusedBaseTensor &operator=(const FusedBaseTensor &other)
    {
#ifdef DEBUG_FUSED_TENSOR
        std::cout << "FusedBaseTensor copy assignment\n";
#endif
        if (this == &other)
        {
#ifdef DEBUG_FUSED_TENSOR
    ...
    FusedBaseTensor &operator=(FusedBaseTensor &&other) noexcept
    {
#ifdef DEBUG_FUSED_TENSOR
        std::cout << "FusedBaseTensor move assignment\n";
#endif
        if (this == &other)
        {
#ifdef DEBUG_FUSED_TENSOR
    ...
    bool operator==(const FusedBaseTensor<T, Dims1...> &other) const
    {
    ...
    FusedBaseTensor transposed(const my_size_t order[sizeof...(Dims)]) const
    {
        FusedBaseTensor outp = *this;
        for (my_size_t i = 0; i < getNumDims(); ++i)
        {
    ...
    FusedBaseTensor transposed(void) const
    {
    ...
    FusedBaseTensor &setToZero(void)
    {
        std::fill_n(data_.data(), totalSize, 0);
        return *this;
    }

    FusedBaseTensor &setHomogen(T _val)
    {
        std::fill_n(data_.data(), totalSize, _val);
        return *this;
    }

    FusedBaseTensor &setRandom(my_size_t _maxRand, my_size_t _minRand)
    {
        for (my_size_t i = 0; i < totalSize; ++i)
        {
    ...
    // for all dimensions
    FusedBaseTensor &setDiagonal(T _val)
    {
    ...
    FusedBaseTensor &setIdentity(void)
    {
    ...
    static FusedBaseTensor I(void)
    {
        static_assert(sizeof...(Dims) >= 2, "Identity requires at least 2 dimensions.");

        static_assert(((Dims == dims[0]) && ...), "All dimensions must be the same for an identity tensor");

        FusedBaseTensor<T, Dims...> _outp;
        _outp.setDiagonal(1);
        return _outp;
    }

    FusedBaseTensor &setSequencial(void)
    {
        for (my_size_t i = 0; i < totalSize; ++i)
        {
            data_[i] = i;
    ...
    void getDiagonalEntries(FusedBaseTensor<T, DiagonalSize, 1> &diagonalEntries) const
    {
    ...
    static FusedBaseTensor einsum(const FusedBaseTensor<T, Dims1...> &_tensor1, const FusedBaseTensor<T, Dims2...> &_tensor2, my_size_t a, my_size_t b)
    {
...
protected:
    ...
    template <my_size_t... Dims1>
    inline void checkDimensionsMismatch(const FusedBaseTensor<T, Dims1...> &other) const
    {
